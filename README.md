...
Игра инициализирует Yandex Games SDK при старте (`/sdk.js` + `YaGames.init()`). Межстраничная реклама показывается при рестарте, проигрыше и после прохождения уровня. Вне площадки Яндекса игра работает как обычно, реклама просто пропускается.
